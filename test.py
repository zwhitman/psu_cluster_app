...
def minDist2Centroids(x, centroids):
    n,k = centroids.shape
    if n == 1:
        dist = dist2Centroid(x, centroids)
    else:
        dists = NUM.apply_along_axis(dist2Centroid, 1, x, centroids)
        dist = dists.min(1)
    return dist

def closestCentroidID(x, centroids):
    dists = NUM.apply_along_axis(dist2Centroid, 1, x, centroids)
    closestID = NUM.argmin(dists, 1)
    return closestID

def kMeansPlusPlus(x, nParts, seeds = None):
    n = len(x)
    if seeds == None:
        seeds = []
        nPart = 0
    ...
    while nPart < nParts:
        if nPart:
            minDist = minDist2Centroids(x, centroids)
        sumDist = minDist.sum()
        probs = minDist / sumDist
        cumSum = probs.cumsum()
        randUni = RAND.uniform()
        newSeed = NUM.argwhere(cumSum > randUni)[0][0]
...
class Partition(object):
    ...
    def optimize(self):
        ...
        if self.aspatial:
            ...
            for row, numPart in enumerate(self.groupList):
                ...
                fRes = (numPart, meanF, minF, maxF, medF)
    ...
    def setSeeds(self, silent = False):
        if self.initMethod == "GET_SEEDS_FROM_FIELD":
            ...
            self.seedIndices = NUM.where(seedData == 1)[0]
            lenSeeds = len(self.seedIndices)
            if lenSeeds == 0:
                #### No Valid Seeds ####
            ...
            if lenSeeds > self.kPartitions:
                ...
                permSeeds = RAND.permutation(self.seedIndices)
                self.seedIndices = permSeeds[0:self.kPartitions]
            ...
            if lenSeeds < self.kPartitions:
                ...
                seedList = list(self.seedIndices.flatten())
                self.seedIndices = kMeansPlusPlus(self.z, self.kPartitions,
                                                  seeds = seedList)
            ...
            self.seedIndices = randInd[0:self.kPartitions]
        self.partIDs = range(self.kPartitions)
        self.seeds = NUM.zeros(self.n, dtype = int)
        self.seeds[self.seedIndices] = 1

    def aspatialSolve(self, finalize = True):
        feasible = True
        ARCPY.SetProgressor("default", ARCPY.GetIDMessage(84259))
        centroids = self.z[self.seedIndices]
        initial = True
        flag = 1
        iters = 0
        while flag:
            if initial:
                closest = closestCentroidID(self.z, centroids)
                initial = False
            for part in self.partIDs:
                idsInCentroid = NUM.where(closest == part)
                newCentroid = getCentroid(self.z, idsInCentroid)
                centroids[part] = newCentroid
            newClosest = closestCentroidID(self.z, centroids)
            if NUM.all(newClosest == closest):
                flag = 0
            else:
                closest = newClosest
                iters += 1
            if iters > aSpatialIters:
                feasible = False
                flag = 0
    ...
    def minSpanTree(self):
        ...
        self.partition = NUM.zeros((numObs,), int)
        ...
        if self.spaceConcept in contTypes:
            ...
            contiguity = NUM.zeros((self.ssdo.numObs,), int)
            ...
            if islandsPresent:
                if self.numNeighs:
                    ...
                    for row in xrange(N):
                        ...
                        if masterID in islands:
                            orderID = self.ssdo.master2Order[masterID]
                            for nh in gaSearch:
                                neighID = gaTable[nh.idx][2]
                                neighOrderID = self.ssdo.master2Order[neighID]
                                iVals = self.z[orderID]
                                jVals = self.z[neighOrderID]
                                dist = ((iVals - jVals)**2.0).sum()
                ...
                if includeIt:
                    ...
                    if nn:
                        ...
                        for neigh in xrange(nn):
                            links.append((orderID, nhIDs[neigh]))
                            jVals = nhVals[neigh]
                            dist = ((iVals - jVals)**2.0).sum()
                            weights.append(dist)
                    else:
            ...
            for row in xrange(self.ssdo.numObs):
                ...
                for nh in gaSearch:
                    ...
                    neighID = gaTable[nh.idx][2]
                    neighOrderID = self.ssdo.master2Order[neighID]
                    if (neighOrderID, orderID) not in links:
                        jVals = self.z[neighOrderID]
                        dist = ((iVals - jVals)**2.0).sum()
    ...
    def createMST(self):
        order = NUM.argsort(self.weights)
        NW = len(self.weights)
        nm1 = self.numFeatures - 1
        linkOrder = self.links[order]
        weightOrder = self.weights[order]
        clusterSets = COLL.defaultdict(set)
        clusterMap = NUM.arange(self.ssdo.numObs)
        mst = NUM.empty((nm1, 2), int)
        treeWeights = NUM.empty((nm1, ), float)
        id2Parts = COLL.defaultdict(int)
        count = 0
        ARCPY.SetProgressor("step", ARCPY.GetIDMessage(84323), 0, NW, 1)
        i = 0
        count = 0
        disconnected = False
        while count < nm1:
            ...
            uClust = clusterMap[u]
            if vClust != uClust:
                uSet = clusterSets[uClust]
                uSet.add(v)
                vSet = clusterSets[vClust]
                vSet.add(u)
                allSet = uSet.union(vSet)
                clusterSets[uClust] = allSet
                for old in allSet:
                    clusterMap[old] = uClust
                del clusterSets[vClust]
                mst[count] = (v,u)
                treeWeights[count] = w
                count += 1
            i += 1
        ...
        clustKeys = clusterSets.keys()
        startingK = len(clustKeys)
        if disconnected:
            #### Inherent Breaks due to Spatial Config ####
            mst = mst[0:count,:]
            treeWeights = treeWeights[0:count]
            disCount = str(len(clustKeys))
            ARCPY.AddIDMessage("WARNING", 1392, disCount)
        ...
        for ind, clust in enumerate(clustKeys):
            ids = clusterSets[clust]
            self.part2IDs[ind] = list(ids)
    ...
    def spatialSolve(self, optimize = False):
        ...
        if (1 < startTree < maxNumGroups + 1) and optimize:
            ...
            msg = ARCPY.GetIDMessage(84326).format(startTree, fStat)
            ARCPY.AddMessage(msg)
        ...
        while len(self.tree) < self.kLimit:
            ...
            if optimize and reportFStat:
                ...
                msg = ARCPY.GetIDMessage(84326).format(nc, fStat)
                ARCPY.AddMessage(msg)
            ARCPY.SetProgressorPosition()
    ...
    def report(self, fileName = None, optimal = False):
        ...
        if fileName:
            ...
            #### Add Global Group ####
            labVals = (self.numFeatures, self.globalSTD, self.globalZDistSum)
            ...
            for group in xrange(self.numGroups):
                ...
                labVals = (group + 1, count, stdDist, ssdVal)
...
def summarizeFStats(fStatRes, groupList):
    ...
    maxGroup = groupList[maxInd]
    rowRes = (ARCPY.GetIDMessage(84261), maxGroup, maxMeanValOut)
    ARCPY.AddMessage("\n"+msg.format(*rowRes))
    ...
    rowRes = (ARCPY.GetIDMessage(84412), groupList[maxMinInd], maxMinValOut)
    ...
    rowRes = (ARCPY.GetIDMessage(84413), groupList[maxMaxInd], maxMaxValOut)
    ARCPY.AddMessage(msg.format(*rowRes))
    ...
    rowRes = (ARCPY.GetIDMessage(84414), groupList[maxMedInd], maxMedValOut)
    ARCPY.AddMessage(msg.format(*rowRes))
